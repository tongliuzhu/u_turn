...
GridSearch::GridSearch(const HybridAstarConfig &open_space_conf) {
  xy_grid_resolution_ = 1 / kMeter2Pixel;
...
bool GridSearch::CheckConstraints(std::shared_ptr<Node2d> node) {
  ...
  if (obstacles_vec_.empty()) {
    return true;
  }
  for (const auto &obstacle : obstacles_vec_) {
    if (DistanceTo(Point2D(node->GetGridX() / kMeter2Pixel,
                           node->GetGridY() / kMeter2Pixel),
                   obstacle) < node_radius_ + obstacle.obstacle_radius) {
      return false;
    }
  }
  return true;
...
bool GridSearch::GenerateAStarPath(
    ...
    const std::vector<ObstacleInfo> &obstacles_vec, GridAStartResult *result) {
...
bool GridSearch::GenerateDpMap(const double ex, const double ey,
                               const std::vector<double> &XYb,
                               const std::vector<ObstacleInfo> &obstacles_vec) {
  std::priority_queue<std::pair<std::string, double>,
                      std::vector<std::pair<std::string, double>>, cmp>
      open_pq;
  std::unordered_map<std::string, std::shared_ptr<Node2d>> open_set;
  dp_map_ = decltype(dp_map_)();
  std::cout << "xy bounds: " << XYb[0] << " " << XYb[1] << std::endl;
  XYbounds_ = XYb;
...
// Distance to round obstacle
double GridSearch::DistanceTo(Point2D p, ObstacleInfo obstacle) const {
  double dx = p.x - obstacle.obstacle_x;
  double dy = p.y - obstacle.obstacle_y;
