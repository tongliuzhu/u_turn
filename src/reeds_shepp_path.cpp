...
ReedShepp::ReedShepp(const VehicleParam &vehicle_param,
                     const HybridAstarConfig &hybrid_astar_conf)
    : vehicle_param_(vehicle_param),
      planner_hybrid_astar_config_(hybrid_astar_conf) {
  max_kappa_ = std::tan(vehicle_param_.max_steer_angle /
                        vehicle_param_.steer_ratio / vehicle_param_.wheel_base);
  std::cout << "max_kappa: " << max_kappa_ << std::endl;
}

std::pair<double, double>
ReedShepp::calc_tau_omega(const double u, const double v, const double xi,
                          const double eta, const double phi) {
  double delta = util::NormalizeAngle(u - v);
  ...
  double tau = 0.0;
  if (t2 < 0) {
    tau = util::NormalizeAngle(t1 + M_PI);
  } else {
    tau = util::NormalizeAngle(t1);
  }
  double omega = util::NormalizeAngle(tau - u + v - phi);
...
void ReedShepp::LSL(const double x, const double y, const double phi,
  ...
  std::pair<double, double> polar =
      util::Cartesian2Polar(x - std::sin(phi), y - 1.0 + std::cos(phi));
  double u = polar.first;
  double t = polar.second;
  double v = 0.0;
  if (t >= 0.0) {
    v = util::NormalizeAngle(phi - t);
    if (v >= 0.0) {
      param->flag = true;
      param->u = u;
      param->t = t;
...
void ReedShepp::LSR(const double x, const double y, const double phi,
  ...
  if (u1 >= 4.0) {
    ...
    t = util::NormalizeAngle(t1 + theta);
    v = util::NormalizeAngle(t - phi);
    if (t >= 0.0 && v >= 0.0) {
      param->flag = true;
      param->u = u;
      param->t = t;
...
void ReedShepp::LRL(const double x, const double y, const double phi,
  ...
  if (u1 <= 4.0) {
    ...
    t = util::NormalizeAngle(t1 + 0.5 * u + M_PI);
    v = util::NormalizeAngle(phi - t + u);
    if (t >= 0.0 && u <= 0.0) {
      param->flag = true;
      param->u = u;
      param->t = t;
...
void ReedShepp::SLS(const double x, const double y, const double phi,
                    RSPParam *param) {
  double phi_mod = util::NormalizeAngle(phi);
  double xd = 0.0;
  double u = 0.0;
  double t = 0.0;
  double v = 0.0;
...
void ReedShepp::LRSR(const double x, const double y, const double phi,
  ...
  double xi = x + std::sin(phi);
  double eta = y - 1.0 - std::cos(phi);
  std::pair<double, double> polar = util::Cartesian2Polar(-eta, xi);
  ...
  if (rho >= 2.0) {
    t = theta;
    u = 2.0 - rho;
    v = util::NormalizeAngle(t + 0.5 * M_PI - phi);
...
void ReedShepp::LRSL(const double x, const double y, const double phi,
  ...
  double xi = x - std::sin(phi);
  double eta = y - 1.0 + std::cos(phi);
  std::pair<double, double> polar = util::Cartesian2Polar(xi, eta);
  double rho = polar.first;
  double theta = polar.second;
  double r = 0.0;
  double t = 0.0;
  ...
  if (rho >= 2.0) {
    ...
    u = 2.0 - r;
    t = util::NormalizeAngle(theta + std::atan2(r, -2.0));
    v = util::NormalizeAngle(phi - 0.5 * M_PI - t);
    if (t >= 0.0 && u <= 0.0 && v <= 0.0) {
      param->flag = true;
      param->u = u;
      param->t = t;
...
bool ReedShepp::GenerateLocalConfigurations(
  ...
  // double step_scaled =
  // planner_hybrid_astar_config_.warm_start_config().step_size() * max_kappa_;
  double step_scaled =
      0.125 * max_kappa_; // distance between each consecutive points
  ...
  for (size_t i = 0; i < shortest_path->segs_types.size(); ++i) {
    ...
    index--;
    if (i >= 1 &&
        shortest_path->segs_lengths.at(i - 1) *
  ...
  for (size_t i = 0; i < px.size(); ++i) {
    ...
    shortest_path->phi.push_back(
        util::NormalizeAngle(pphi.at(i) + start_node->GetPhi()));
  }
